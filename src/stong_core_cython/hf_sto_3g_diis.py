...
def build_Fock_matrix_up(ERImat, hmat, c_up, c_dn, n_up, n_dn):
    Jmat_up = sum(
    ...
    return Fmat_up, Jmat_up, Kmat_up


def build_Fock_matrix_dn(ERImat, hmat, c_up, c_dn, n_up, n_dn):
    Jmat_dn = sum(
        np.einsum("ijkl,k, l->ij", ERImat, c_up[i], c_up[i]) for i in range(n_up)
    ) + sum(np.einsum("ijkl,k, l->ij", ERImat, c_dn[i], c_dn[i]) for i in range(n_dn))
    Kmat_dn = sum(
    ...
    return Fmat_dn, Jmat_dn, Kmat_dn


def scf(zeta_1s, zeta_2sp, df, n_up, n_dn, Z, max_iter=10000, tol=1e-6):
    k = 3
    diis_resid_up = []
    diis_resid_dn = []
    diis_Fock_up = []
    ...
    # 初期化
    # c_up = np.zeros((k, n_up, 5))
    # c_dn = np.zeros((k, n_dn, 5))
    ...
    for i in range(max_iter):
        # Fock行列の構築
        Fmat_up, Jmat_up, Kmat_up = build_Fock_matrix_up(
            ERImat, hmat, c_up, c_dn, n_up, n_dn
        )
        Fmat_dn, Jmat_dn, Kmat_dn = build_Fock_matrix_dn(
            ERImat, hmat, c_up, c_dn, n_up, n_dn
        )
        ...
        # エラーベクトルの計算
        e_up = Fmat_up @ D_up @ Smat - Smat @ D_up @ Fmat_up
        e_dn = Fmat_dn @ D_dn @ Smat - Smat @ D_dn @ Fmat_dn
        print(e_up)
        print(e_dn)

        c_up = 0.9 * c_up + 0.1 * vec_up[:, :n_up].T
        c_dn = 0.9 * c_dn + 0.1 * vec_dn[:, :n_dn].T
        D_up = vec_up[:, :n_up] @ vec_up[:, :n_up].T
        D_dn = vec_dn[:, :n_dn] @ vec_dn[:, :n_dn].T
        # c_up = vec_up[:, :n_up].T
        # c_dn = vec_dn[:, :n_dn].T

        # resid_up = np.dot(np.dot(c_up, val_up[:n_up]), c_up.T) - Smat
